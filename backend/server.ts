...
io.on("connection", (socket) => {
  ...
  socket.on("joinLobby", async (gameId) => {
    try {
      if (activeGames[gameId]) {
        ...
        game.addPlayer();
        socket.join(gameId);
        socket.emit("joinedLobby", { gameId, playerId: game.players.length });
        io.to(gameId).emit("playerJoined", { playerId: game.players.length });
      ...
      try {
        ...
        if (player) {
          ...
          if (tile.event.type !== 0) {
            ...
            io.to(gameId).emit("tileEventTriggered", {
              playerId,
              eventType: tile.event.type,
            });
          }
        }
      } catch (error) {
  ...
  socket.on("movePlayerDiceRoll", async (gameId: string, playerId: number) => {
    ...
    try {
      ...
      if (player) {
        ...
        console.log(
          `Player ${playerId} moved by dice roll to position ${newPosition} in game ${gameId}`
        );
        // Check for tile event
        const tile = activeGames[gameId].map.tiles[newPosition];
        if (tile.event.type !== 0) {
  ...
  socket.on(
    "startBattle",
    (gameId: string, player1Id: number, player2Id: number) => {
      if (!activeGames[gameId]) {
        socket.emit("error", { message: "Game does not exist" });
        return;
      }
      ...
      if (player1 && player2) {
        ...
        game.currentBattle = battle; // Store the battle in the game instance
        io.to(gameId).emit("battleStarted", { player1Id, player2Id });
        console.log(
          `Battle started between Player ${player1Id} and Player ${player2Id}`
        );
      } else {
  ...
  socket.on(
    "processTurn",
    (gameId: string, playerId: number, action: string) => {
      const game = activeGames[gameId];
      ...
      const opponent =
        battle.player.id === playerId ? battle.opponent : battle.player;

      if (!battle.playerAction) {
        battle.playerAction = { playerId, action };
      } else if (!battle.opponentAction) {
        battle.opponentAction = { playerId, action };

        // Process the turn when both players have chosen actions
        const result = battle.processTurn(
          battle.playerAction.action,
          battle.opponentAction.action
        );

        io.to(gameId).emit("turnProcessed", {
          result,
          player1Hp: battle.player.health, // Assuming `health` is a property of Player
          player2Hp: battle.opponent.health,
        });

        // Reset actions for the next turn
        battle.playerAction = null;
        battle.opponentAction = null;
      }
    }
  );

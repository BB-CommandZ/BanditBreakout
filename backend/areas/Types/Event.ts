...
export interface IEvent {
    ...
    effect: string;
    tile: Tile;
    onStep(playerId: number, game: Game, totalStepsRemainingInSequence: number): Promise<void>;
}
...
export class BattleEvent implements IEvent {
    ...
    public async onStep(playerId: number, game: Game, totalStepsRemainingInSequence: number): Promise<void> {
        ...
        console.log(`Player ${playerId} stepped on tile ${this.tile.index} and is ambushed by Wim!`);
        const wimNPC = game.createNPC('Wim_Battle');
        if (wimNPC) {
            await game.startBattle(player, wimNPC);
        } else {
            console.error("Failed to create Wim NPC for battle.");
        }
    }
}
...
export class BattleEffectEvent implements IEvent {
    ...
    public async onStep(playerId: number, game: Game, totalStepsRemainingInSequence: number): Promise<void> {
        ...
        const availableEffects = ["Sunscreen", "Food", "Cactus", "Revolver", "Cowboy_Boots"];
        const effectToGive = availableEffects[Math.floor(Math.random() * availableEffects.length)]; // Random effect

        console.log(`Player ${player.id} stepped on a Battle Effect Tile! Attempting to give: ${effectToGive}`);
        if (player.status.addBattleEffect(effectToGive)) {
            console.log(`You gained the '${effectToGive}' battle effect! It will apply in your next battle.`);
        } else {
            console.log(`You already have the '${effectToGive}' battle effect (or another active effect prevents stacking). No change.`);
        }
    }
}
...
export class ShopEvent implements IEvent {
    ...
    public async onStep(playerId: number, game: Game, totalStepsRemainingInSequence: number): Promise<void> {
        ...
        // Emit socket event to open the shop UI on the frontend
        game.io.to(game.game_id).emit('requestShopChoice', {
            playerId: playerId,
            shopItems: this.shopItems,
            playerGold: player.getGold(),
            playerInventoryCount: player.inventory.items.length // Send current item count
        });

        // Wait for shopChoiceMade event from frontend
        const choice = await new Promise<{ itemId?: number, action: 'buy' | 'exit' }>(resolve => {
            const handler = (data: { playerId: number, itemId?: number, action: 'buy' | 'exit' }) => {
                if (data.playerId === playerId) {
                    game.io.off('shopChoiceMade', handler); // Remove listener
                    resolve(data);
                }
            };
            game.io.on('shopChoiceMade', handler);
        });

        if (choice.action === 'buy' && typeof choice.itemId !== 'undefined') {
            const selectedShopItem = this.shopItems.find(item => item.id === choice.itemId);
            if (selectedShopItem) {
                if (player.getGold() >= selectedShopItem.price) {
                    if (player.inventory.canAddItem()) {
                        player.gold(`-${selectedShopItem.price}`); // Deduct gold
                        player.inventory.obtain(selectedShopItem.id); // Add item
                        console.log(`Player ${playerId} bought a ${selectedShopItem.name} for ${selectedShopItem.price} gold.`);
                        console.log(`Remaining gold: ${player.getGold()}`);
                        // Optionally, emit event to update UI with new gold/inventory
                    } else {
                        console.log(`Player ${playerId} tried to buy ${selectedShopItem.name} but their inventory is full!`);
                    }
                } else {
                    console.log(`Player ${playerId} doesn't have enough gold for ${selectedShopItem.name}.`);
                }
            } else {
                console.warn(`Player ${playerId} sent invalid item ID ${choice.itemId} for purchase.`);
            }
        } else if (choice.action === 'exit') {
            console.log(`Player ${playerId} exited the shop.`);
        }
        // If action is neither buy nor exit, or itemId is missing for buy, nothing happens.
    }
}
...
export class SlotsEvent implements IEvent {
    ...
    public async onStep(playerId: number, game: Game, totalStepsRemainingInSequence: number): Promise<void> {
        const player = game.players.find(p => p.id === playerId);
        if (!player) { return; }
        if (totalStepsRemainingInSequence === 0) { // Only play slots on final landing
...
export class MiningEvent implements IEvent {
    ...
    public async onStep(playerId: number, game: Game, totalStepsRemainingInSequence: number): Promise<void> {
        const player = game.players.find(p => p.id === playerId);
        if (!player) { return; }
        if (totalStepsRemainingInSequence === 0) { // Only mine on final landing
    ...
    public async onStep(playerId: number, game: Game, totalStepsRemainingInSequence: number): Promise<void> {
        ...
        if (!decisionData) {
            ...
            if (possiblePaths.length === 1) {
                ...
                // Pass 0 for totalStepsRemainingInSequence as this is a single move to a final destination in this fallback
                await player.move.to(possiblePaths[0], 0);
                return;
            }
            
            // Emit socket event to request decision from frontend
            game.io.to(game.game_id).emit('requestDecision', {
                playerId: playerId,
                dialogues: ["Choose your path:"], // Simple dialogue for fallback
                choices: possiblePaths.map(pathIndex => ({ text: `Go to Tile ${pathIndex}`, tileId: pathIndex }))
            });

            // Wait for decisionMade event from frontend
            const chosenTileId = await new Promise<number>(resolve => {
                const handler = (data: { playerId: number, chosenTileId: number }) => {
                    if (data.playerId === playerId) {
                        game.io.off('decisionMade', handler); // Remove listener
                        resolve(data.chosenTileId);
                    }
                };
                game.io.on('decisionMade', handler);
            });

            console.log(`Player ${playerId} chose path to Tile ${chosenTileId}.`);
            player.status.setResolvedDecisionPath(chosenTileId);
            // The remaining steps logic is handled in Movement.ts
            // We don't need to set remaining steps here.

            return; // Exit after handling decision

        }

        console.log(`\n--- Decision on Tile ${this.tile.index} ---`);
        // Emit dialogue lines one by one or all at once, depending on frontend needs
        // For now, emit all dialogue and choices
        game.io.to(game.game_id).emit('requestDecision', {
            playerId: playerId,
            dialogues: decisionData.dialogues,
...
export class BossBattleEvent implements IEvent {
    ...
    public async onStep(playerId: number, game: Game, totalStepsRemainingInSequence: number): Promise<void> {
        ...
        console.log(`\n !!! Player ${player.id} (${player.getCharacterName()}) has reached Tile ${this.tile.index} !!!`);
        console.log("The air grows heavy... SPINDLE, the Bandit King, appears!");
        // Optional: Spindle dialogue

        const spindleNPC = game.createNPC('Spindle_Boss');
        if (spindleNPC) {
            const battleOutcome = await game.startBattle(player, spindleNPC, { isBossBattle: true }); // Pass context

            if (battleOutcome.winner === player) {
                console.log(`\n🎉🎉🎉 ${spindleNPC.name} has been defeated! 🎉🎉🎉`);
                // Display character-specific victory message/cutscene
                const characterName = player.getCharacterName();
                if (characterName === "Buckshot") console.log("Buckshot: Justice served, the old-fashioned way.");
                // ... other character victory lines ...
                else console.log(`${characterName} has brought peace back to the frontier!`);

                game.setWinner(player);
            } else { // Player lost to Spindle
                console.log(`\n☠️ You have been defeated by ${spindleNPC.name}! ☠️`);
                // Spindle loss consequences are handled in Game.initiateBattle now
                console.log("Spindle cackles. 'You'll never best me!'");
            }
        } else {
            console.error("Critical Error: Failed to create Spindle NPC! Awarding win as fallback.");
            game.setWinner(player);
        }
    }
}

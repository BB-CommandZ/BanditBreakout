...
export default class Status {
    ...
    public activeBattleEffects: Set<string>;
    public shield: number;
    public isDefending: boolean;

    constructor(initialGold: number = 10) {
        this.health = 10;
        this.activeBattleEffects = new Set();
        this.shield = 0;
    ...
    public addBattleEffect(effectName: string): boolean {
        if (this.activeBattleEffects.has(effectName)) {
            return false; // Already has the effect
        }
        this.activeBattleEffects.add(effectName);
        return true;
    }

    public hasBattleEffect(effectName: string): boolean {
        return this.activeBattleEffects.has(effectName);
    }

    public consumeAllBattleEffects(): void {
        this.activeBattleEffects.clear();
    }
    ...
    public getSaveData() {
        return {
            ...
            activeBattleEffects: Array.from(this.activeBattleEffects),
            shield: this.shield,
    ...
    public loadFromSave(saveData: any) {
        ...
        this.activeBattleEffects = new Set(saveData.activeBattleEffects || []);
        this.shield = saveData.shield || 0;
        this.isDefending = saveData.isDefending || false;
    }
}

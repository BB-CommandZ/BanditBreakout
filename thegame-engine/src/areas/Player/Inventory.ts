...
export default class Inventory {
    ...
    public removeRandomItem(): IBaseItem | undefined {
        if (this.items.length === 0) {
            return undefined;
        }
        const randomIndex = Math.floor(Math.random() * this.items.length);
        const removedItem = this.items.splice(randomIndex, 1)[0];
        console.log(`Player ${this.player.id} lost item: ${removedItem.name}.`);
        return removedItem;
    }

    public addItem(item: IBaseItem): boolean {
        if (this.canAddItem()) {
            this.items.push(item);
            console.log(`Player ${this.player.id} gained item: ${item.name}.`);
            return true;
        }
        console.log(`Player ${this.player.id}'s inventory is full (Max ${this.MAX_ITEMS}). Cannot add item: ${item.name}.`);
        return false;
    }
